...
const normalizeVec3 = (v) => {
  const a = Math.sqrt(v.x*v.x + v.y*v.y + v.z*v.z)
  return mulVec3(1/a, v)
}
...
const calcCentroidCoordinates = (v1, v2, v3, p) => {
  const s = calcTriangleArea(v1, v2, v3)
  const c1 = calcTriangleArea(v2, v3, p) / s
  const c2 = calcTriangleArea(v3, v1, p) / s
  const c3 = calcTriangleArea(v1, v2, p) / s
  return {c1, c2, c3}
}
