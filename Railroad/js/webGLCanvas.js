...
webGLCanvas = function ( canvasID )
{
    ...
    this.canvas.addEventListener("mousedown", function (event) {
        ...
        let id2 = this.objectIDMap[index + 1];
        let id3 = this.objectIDMap[index + 2];
        let hitObjName = null;
        
        if(id1 != 0)
        {
            hitObjName = this.objectIDList[id1];
        }

        if( this.mouseDownCallback )
        {
            this.mouseDownCallback( x, y, hitObjName, id2, id3, event.button );
        }
    }.bind(this));
    ...
    this.canvas.addEventListener("touchstart", function (event)
    {
        ...
        let index = ( ( this.canvas.height - Math.ceil(y) ) * this.canvas.width + Math.ceil(x) ) * 4;
        let id = this.objectIDMap[index + 0];
        let hitObjName = null;
        if(id != 0)
        {
